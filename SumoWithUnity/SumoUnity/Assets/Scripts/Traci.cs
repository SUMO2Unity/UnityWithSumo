...
public class Traci : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        egoVehicle.transform.position = new Vector3((float)shape.X, 0.001270017f, (float)shape.Y);
    ...
    {
        // Close the connection to SUMO when quitting the application
        //client.Vehicle.Remove(vehicleID, 0);
    ...
    void FixedUpdate()
    {
        ...
        for (int i = 0; i < newvehicles.Count; i++)
        {
            ...
            newcar.transform.position = new Vector3((float)newcarposition.X, 0.0f,
